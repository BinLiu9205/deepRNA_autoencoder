...
with open('/mnt/dzl_bioinf/binliu/deepRNA/deepRNA/Experimental_update_revision/Hyperparameter_search_for_structure/training/conf/model_structure_sweep_round1.yaml', 'r') as file:
    sweep_config = yaml.safe_load(file)

@hydra.main(config_path="conf", config_name="model_structure_search")
def main(cfg: DictConfig) -> None:
    flat_cfg = OmegaConf.to_container(cfg, resolve=True)
    wandb.init(project='deepRNA', config=flat_cfg)
    #wandb.init(project='deeprna/deepRNA', config=flat_cfg)
...
def run_sweep():
    ...
    wandb.agent(sweep_id, function=main, count = 15) #count=10)
    # Uncomment the following if want to to continue an existing one (changing ID)
    # wandb.agent('deeprna/deepRNA/myfogr9r', function=main,count = 15)
