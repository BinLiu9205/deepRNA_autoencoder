...
for latent_mu_name in latent_mu_names:
    ...
    latent_cancer_pca_biplot = latent_fig_save_path_base + '/lung_vs_breast_adenocarcinoma_phenotype_pca_biplots_twoDimensions'
    os.makedirs(latent_cancer_pca_biplot, exist_ok=True)
    latent_cancer_pca_biplot = latent_cancer_pca_biplot + '/' + shortened
    
    latent_cancer_deDims_plot = latent_fig_save_path_base + '/lung_vs_breast_adenocarcinoma_phenotype_deDims'
    os.makedirs(latent_cancer_deDims_plot, exist_ok = True)
    latent_cancer_deDims_plot = latent_cancer_deDims_plot + '/' + shortened
    
    latent_cancer_heatmap_plot = latent_fig_save_path_base + '/lung_vs_breast_adenocarcinoma_phenotype_latent_heatmaps'
    os.makedirs(latent_cancer_heatmap_plot, exist_ok = True)
    latent_cancer_heatmap_plot = latent_cancer_heatmap_plot + '/' + shortened
    
    latent_tables = latent_fig_save_path_base + '/lung_vs_breast_adenocarcinoma_phenotype_results'
    ...
    latentDim.pca_biplots_based_on_selected_indices(latent_file,  breast_index, lung_index, 'Breast Cancer', 'Lung Cancer', anno, latent_cancer_pca_biplot)
    
    df = latentDim.differential_expressed_latent_dimensions_with_given_indices(latent_file, prior_res, breast_index, lung_index, 'Breast Cancer', 'Lung Cancer', anno, latent_cancer_deDims_plot)
    
    latentDim.deg_latent_dimensions_with_given_indices_heatmap_conditions(latent_file, breast_index, lung_index, 'Breast Cancer', 'Lung Cancer', anno, latent_cancer_heatmap_plot)
