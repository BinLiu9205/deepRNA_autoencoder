...
for latent_mu_name in latent_mu_names:
    ...
    latent_cancer_pca_biplot = latent_fig_save_path_base + '/healthy_adenocarcinoma_pca_biplots_twoDimensions'
    os.makedirs(latent_cancer_pca_biplot, exist_ok=True)
    latent_cancer_pca_biplot = latent_cancer_pca_biplot + '/' + shortened
    
    latent_cancer_deDims_plot = latent_fig_save_path_base + '/healthy_adenocarcinoma_deDims'
    os.makedirs(latent_cancer_deDims_plot, exist_ok = True)
    latent_cancer_deDims_plot = latent_cancer_deDims_plot + '/' + shortened
    
    latent_cancer_heatmap_plot = latent_fig_save_path_base + '/healthy_adenocarcinoma_latent_heatmaps'
    os.makedirs(latent_cancer_heatmap_plot, exist_ok = True)
    latent_cancer_heatmap_plot = latent_cancer_heatmap_plot + '/' + shortened
    
    latent_tables = latent_fig_save_path_base + '/healthy_adenocarcinoma_results'
    ...
    latentDim.pca_biplots_based_on_selected_indices(latent_file, normal_index, adenocarcinoma_index, 'Healthy', 'Adenocarcinoma', anno, latent_cancer_pca_biplot)
    
    df = latentDim.differential_expressed_latent_dimensions_with_given_indices(latent_file, prior_res, normal_index, adenocarcinoma_index, 'Healthy', 'Adenocarcinoma', anno, latent_cancer_deDims_plot)
    
    latentDim.deg_latent_dimensions_with_given_indices_heatmap_conditions(latent_file, normal_index, adenocarcinoma_index, 'Healthy', 'Adenocarcinoma', anno, latent_cancer_heatmap_plot)
